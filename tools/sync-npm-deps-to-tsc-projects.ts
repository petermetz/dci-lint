...
const main = async (argv: string[], env: NodeJS.ProcessEnv) => {
  ...
  tsConfigPaths.forEach((it) => {
    const pkgName = "@dci-lint/".concat(path.basename(path.dirname(it)));
  ...
  for (const pkgJsonPath of pkgJsonPaths) {
    ...
    tsConfig.compilerOptions.outDir = "./dist/lib/";
    tsConfig.compilerOptions.declarationDir = "dist/lib";
